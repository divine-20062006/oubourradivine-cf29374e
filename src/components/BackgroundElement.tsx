
import { useEffect, useState } from "react";

const BackgroundElement = () => {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    setIsVisible(true);
  }, []);

  return (
    <div className="fixed inset-0 -z-20 overflow-hidden pointer-events-none">
      {/* Image d'arrière-plan */}
      <div 
        className={`absolute inset-0 bg-cover bg-center bg-no-repeat transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
        style={{ 
          backgroundImage: "url('/lovable-uploads/ca242fb0-3091-4e71-8b8d-4896fb618601.png')" 
        }}
      >
        {/* Overlay pour assurer la lisibilité du texte */}
        <div className="absolute inset-0 bg-[#0d0e17]/70"></div>
      </div>
      
      {/* Éléments de cybersécurité supplémentaires pour renforcer l'ambiance tech */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-0 left-0 w-full h-full" 
             style={{ 
               backgroundImage: "url('data:image/svg+xml;base64,')",
               backgroundSize: "200px 200px"
             }}
        ></div>
      </div>
      
      {/* Éléments lumineux qui rappellent l'image */}
      <div className={`absolute top-20 right-10 w-64 h-64 rounded-full bg-[#0AFFFF]/5 blur-3xl animate-pulse transition-all duration-1000 ${isVisible ? 'opacity-30' : 'opacity-0'}`}></div>
      <div className={`absolute bottom-10 left-20 w-80 h-80 rounded-full bg-[#FF00FF]/5 blur-3xl animate-pulse transition-all duration-1000 delay-700 ${isVisible ? 'opacity-20' : 'opacity-0'}`}></div>
      <div className={`absolute top-40 left-40 w-60 h-60 rounded-full bg-[#FFFF00]/5 blur-3xl animate-pulse transition-all duration-1000 delay-300 ${isVisible ? 'opacity-20' : 'opacity-0'}`}></div>
    </div>
  );
};

export default BackgroundElement;
