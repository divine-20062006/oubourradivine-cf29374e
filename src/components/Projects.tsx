...
const ProjectCard = React.forwardRef<HTMLDivElement, { project: typeof projects[0]; index: number; onClick: () => void }>(
  ({ project, index, onClick }, ref) => (
    ...
    >
      ...
      <div className="p-5 sm:p-6">
        <h3 className="text-base sm:text-lg font-semibold text-white group-hover:text-cyan-300 transition-colors mb-1">{project.title}</h3>
  ...
  return (
    <section id="projets" className="py-16 sm:py-20 lg:py-24">
      <div className="section-container">
        ...
        {/* Première rangée - 3 projets */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mb-5 sm:mb-6">
          {projects.slice(0, 3).map((project, index) => (
            <ProjectCard key={project.id} project={project} index={index} ref={(el) => (cardsRef.current[index] = el)} onClick={() => project.hasDocumentation && setSelectedProject(project)} />
          ))}
        </div>
